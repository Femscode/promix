...
<select id="classFilter">
        @foreach (App\Models\Common\StudentClass::get() as $class )
    ...
    <x-table id="studentsTable">
        ...
        <x-table.thead>
            <x-table.tr>
                ...
                @stack('class_start')
                @if ((! $hideCountry) || (! $hideCurrencyCode))
                <x-table.th class="{{ $classOpenAndOverdue }}">
                    @stack('class_th_start')
                    @if (! $hideCountry)
                    <x-slot name="first">
                        <x-sortablelink column="class" title="{{ trans_choice($studentClass, 1) }}" />
                    </x-slot>
                    @endif
                    @stack('class_th_end')
        ...
        <x-table.tbody>
            ...
            <x-table.tr data-class="{{ $item->class->name ?? '' }}"  href="{{ route($routeButtonShow, $item->id) }}">
                ...
                @stack('class_start')
                @if ((! $hideCountry) || (! $hideCurrencyCode))
                <x-table.td class="{{ $classOpenAndOverdue }}">
                    @stack('student_class_td_start')
                    @if (! $hideCountry)
                    <x-slot name="first">
